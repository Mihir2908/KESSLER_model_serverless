...
class TestNNTrainAndPredictCDMs(unittest.TestCase):
    def test_train_and_predict_future_cdms(self):
        ...
        cdms_1 = forward_test.test_forward_and_plot_cdms()
        ...
        if os.path.exists("synthetic_cdms_1.csv"):
            df = pd.read_csv("synthetic_cdms_1.csv")
        ...
        event = Event([cdm for cdm in cdms_1])
        ...
        # Train the model
        model.learn(event_set, epochs=2, lr=1e-3, batch_size=2, device='cpu', valid_proportion=0)
